...
const part1 = () => {
  ...
  for (let r = 0; r < rows; r++) {
    ...
    for (let c = 0; c < cols; c++) blockingHeight = checkTreeVisibility(r, c, blockingHeight)

    blockingHeight = -Infinity
    for (let c = cols - 1; c; c--) blockingHeight = checkTreeVisibility(r, c, blockingHeight)
  }

  for (let c = 0; c < cols; c++) {
    blockingHeight = -Infinity
    for (let r = 0; r < rows; r++) blockingHeight = checkTreeVisibility(r, c, blockingHeight)

    blockingHeight = -Infinity
    for (let r = rows - 1; r; r--) blockingHeight = checkTreeVisibility(r, c, blockingHeight)
  }
...
const part2 = () => {
  const heightLastAt = Array<number>(10)
  const rows = input.length
  const cols = input[0].length
  const scores = Array<number[]>(rows)
  for (let r = 0; r < rows; r++) scores[r] = Array<number>(cols).fill(1)
  ...
  for (let r = 0; r < rows; r++) {
    heightLastAt.fill(0)
    pos = 0
    for (let c = 0; c < cols; c++) factorTreeView(r, c)

    heightLastAt.fill(0)
    pos = 0
    for (let c = cols - 1; c; c--) factorTreeView(r, c)
  }

  for (let c = 0; c < cols; c++) {
    heightLastAt.fill(0)
    pos = 0
    for (let r = 0; r < rows; r++) factorTreeView(r, c)
